...
class CREATE_STORY :
    ...
    def create_book(self) :
        selected_option = self.task['selected_option']
        previous_story = self.task['previous_stoy_en']
        name = self.task['name']
        age = self.task['age']
        sex = self.task['sex']
        ...
        prompt = f"Children's bookmaking info: {{selected_option: {selected_option}, previous_story:{previous_story}, Name: {{name}}, Age: {age}, Gender: {sex}, Subject: {theme}, Favorite: {favorite}, Teaching: {teach}}} use this info when you make book"
        with open('create_book/prompts/story_other.txt', 'r', encoding='UTF-8') as f:
            sys_prompt = f.read()
